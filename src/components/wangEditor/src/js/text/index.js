...
Text.prototype = {
	...
	_enterKeyHandle: function() {
		...
		function insertEmptyP($selectionElem, e) {
			// 添加有可能出现的软回车？
			let insertStr = e.shiftKey && e.keyCode == 13 ? '<p></p>' : '<p><br></p>';
			let $p = $(insertStr);
		...
		function pHandle(e) {
			...
			// --------------- 设置为引用模块时换行 --------------- //
			if (nodeName === 'FIGURE') {
				$('.quote_block').removeClass('active');
				let $p = $('<p><br></p>');
	...
	_pasteHandle: function() {
		...
		const pasteFilterToPureText = config.pasteFilterToPureText;
		...
		$textElem.on('paste', e => {
			...
			if (pasteFilterToPureText) {
				// 纯文本不能有<br>标签
				// 提供给自由创作使用
				pasteText = pasteText.replace(/<br.*?>/gm, '\n');
			} else {
	...
	_tabHandle: function() {
		...
		$textElem.on('keydown', e => {
			...
			if (selectionNodeName === 'CODE' && parentNodeName === 'PRE') {
				...
				// -------------------- //
				// 修改nbsp；至&emsp;
				editor.cmd.do('insertHTML', '&emsp;&emsp;');
				// -------------------- //
			}
	...
	// ------------modify--------------//
	// 引用点击触发
	_quoteHandle: function _quoteHandle() {
		var editor = this.editor;
		var $textElem = editor.$textElem;
		// 为引用模块增加 selected 样式
		$textElem.on('click', '.reference', function(e) {
			var reference = $(this).children()[0];
			...
			// 修改选区并 restore ，防止用户此时点击退格键，会删除其他内容
			editor.selection.createRangeByElem($reference);
			editor.selection.restoreSelection();

			return true;
		});
		$textElem.on('click', '.quote_block-router', function(e) {
			var $reference = $(this);

			$('.quote_block').removeClass('active');
			$reference.parent().addClass('active');

			// 修改选区并 restore ，防止用户此时点击退格键，会删除其他内容
			editor.selection.createRangeByElem($reference.parent());
			editor.selection.restoreSelection();
